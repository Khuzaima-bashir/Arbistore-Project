...
from arbistore.constants import COLORS_CHOICES


def create_or_return_category(category_name):
...
def create_or_return_sub_categories(category, subcategories):
    sub_categories = []

    for sub_category_name in subcategories:
        sub_category = SubCategories.objects.filter(name=sub_category_name).first()
...
def create_or_return_brand(brand_name):
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        brand = create_or_return_brand("Musto")

        for product_item in product_data:
            category = create_or_return_category(product_item['Category'])
            sub_categories = create_or_return_sub_categories(category, product_item['Sub Categories'])
